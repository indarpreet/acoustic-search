...
public class SearchUserController {
    ...
    public ResponseEntity<List<UserDetails>> getDropDownResults(@RequestBody SearchTerm searchTerm) {
        try {
            ...
            return new ResponseEntity<List<UserDetails>>(searchUserService.getDropDownResults(searchTerm),
                    HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * The method is to get all the employees/user for selected criteria in search
     * engine
     * 
     * @param searchTerm
     * @return {@link List} of {@link UserDetails}
